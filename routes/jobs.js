...
router.get("/", async function (req, res, next) {
  try {
    // let jobs;
    // if (req.body.data) {
    //     jobs = await Job.filter(req.body.data);
    // } else {
    //     jobs = await Job.findAll();
    // }
    let jobs = req.body.data ? await Job.filter(req.body.data) : await Job.findAll();
...
/** GET /[id]  =>  { job }
 *
 *  job is { id, title, salary, equity, companyHandle, jobs }
 *
 * Authorization required: none
 */
...
//     const jobs = await Job.filter(req.body.data);
//     return res.json({ jobs });
//   } catch (err) {
//     return next(err);
//   }
